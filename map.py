...
action2rotation = [0,10,-10]#action = 0 => no rotation,action = 1 => rotate 20 degree, action = 2 => rotate -20 degree
...
def init():
    global sand #Sand is an array that has as many cells as our graphic interface has pixels.
    global goal_x #x-coordinate of the goal(where car has to go,that is the airport or downtown)
    global goal_y #y-coordinate of the goal
    sand = np.zeros((longueur,largeur))#Initializing the sand array with only zeros
    goal_x = 20  #The goal to reach is upper left of the  map
    goal_y = largeur + 20  # The goal to reach is uper left of the map
...
class Car(Widget):
    ...
    def move(self,rotation):
        ...
        self.signal1 = int(np.sum(sand[int(self.sensor1_x)-10:int(self.sensor1_x)+10,int(self.sensor1_y)-10:int(self.sensor1_y)+10]))
        self.signal2 = int(np.sum(sand[int(self.sensor2_x)-10:int(self.sensor2_x)+10,int(self.sensor2_y)-10:int(self.sensor2_y)+10]))
        self.signal3 = int(np.sum(sand[int(self.sensor3_x)-10:int(self.sensor3_x)+10,int(self.sensor3_y)-10:int(self.sensor3_y)+10]))
        
        if(self.sensor1_x > longueur -10) or (self.sensor1_x < 10) or (self.sensor1_y > largeur -10) or (self.sensor1_y < 10):
            self.signal1 = 1. #sensor1 detects full sand
            
        if(self.sensor2_x > longueur -10) or (self.sensor2_x < 10) or (self.sensor2_y > largeur -10) or (self.sensor2_y < 10):
            self.signal2 = 1. #sensor1 detects full sand
            
        if(self.sensor3_x > longueur -10) or (self.sensor3_x < 10) or (self.sensor3_y > largeur -10) or (self.sensor3_y < 10):
            self.signal3 = 1. #sensor1 detects full sand
...
class Game(Widget):
    ...
    def update(self,dt):
        ...
        xx = goal_x - self.car.x #Difference of x-coordinates of goal and car points
        yy = goal_y - self.car.y #Difference of y-coordinates of goal and car points
        
        orientation = Vector(*self.car.velocity).angle((xx, yy)) / 180.  #Direction of the car with respect to the goal and the car
...
class CarApp(App):
    ...
    def load(self,obj):
        print("Loaging last saved brain...\n")
        brain.load()    
